...
from preprocessing.rule_parser.options import *


class RepetitionStrategy(GenericStrategy):
    ...
    def __init__(self, min_loop_time: int = 100, max_loop_time: int = 300, proto: str = 'http'):
        ...
        # MaxLoopTime specified by the user to limit the maximum times of repetition
    ...
    def _generate(self, buffers, last_option) -> tuple[tuple[bytes, bytes], tuple[bytes, bytes]]:
        ...
        return (
            ...
            (
                PacketConstructor(incomplete, self.proto, 'REQUEST').get_packet().encode(codec),
                PacketConstructor(incomplete, self.proto, 'RESPONSE').get_packet().encode(codec)
            ),
            # complete packet, i.e., with the last option
            (
                PacketConstructor(complete, self.proto, 'REQUEST').get_packet().encode(codec),
                PacketConstructor(complete, self.proto, 'RESPONSE').get_packet().encode(codec)
            )
        )
