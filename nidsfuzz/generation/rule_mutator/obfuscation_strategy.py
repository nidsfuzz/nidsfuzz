...
class ObfuscationStrategy(GenericStrategy):
    ...
    def __init__(self, max_replace_time: int = 1, max_insert_time: int = 3, proto: str = 'http'):
        ...
        # MaxLoopTime specified by the user to limit the maximum times of obfuscation
    ...
    def _generate(self, buffers: list[BufferConstructor]):
        ...
        return (
            PacketConstructor(res, self.proto, 'REQUEST').get_packet().encode(codec),
            PacketConstructor(res, self.proto, 'RESPONSE').get_packet().encode(codec),
        )
