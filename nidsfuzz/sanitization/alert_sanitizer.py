...
from sanitization import logger
...
class AlertSanitizer:
    ...
    produced by some NIDSs but are absent in others.
    """
    ...
    def __init__(self,
        ...
        # The variables related to the alert monitor thread
    ...
    def sanitize(self, port_window: list[int]):
        self._pause()
        if self.test_queue.qsize() >= self.port_window_size:
            while self.test_queue.qsize() > 5:
                ...
                if self._is_consistent(selected_rules=rule_id, triggered_alerts=aligned_alerts):
                    logger.debug(f'all targets generated the same alerts for the test case: {rule_id}')
                    continue
                logger.success(f'found an alert discrepancy: {rule_id}')
                self.sanitized_test_queue.put((rule_id, initiator, responder, request, response, aligned_alerts))
        self._resume()

    def _align(self, initiator: tuple[str, int], responder: tuple[str, int], port_window: list[int]) -> dict[str, list[tuple]]:
    ...
    def _is_consistent(self, selected_rules: str, triggered_alerts: dict[str, list[tuple]]):

        # The rules selected to test in this fuzzing iteration
        selected_rule_list: set[str] = set(selected_rules.split(', '))

        # The rules fired by each NIDS
        fired_rules: list[list[str]] = []
        for alerts in triggered_alerts.values():
            fired_rules.append([alert[0] for alert in alerts])

        # Oracle 1: an alert maps to an unselected rule
        for rules in fired_rules:
            if any(rule not in selected_rule_list for rule in rules):
                logger.success(f'an alert maps to an unselected rule: {selected_rules}')
                return False  # Found an alert discrepancy, not a normal behavior

        # Oracle 2: any two NIDSs produce inconsistent alerts
        counters = [Counter(rules) for rules in fired_rules]
        is_equal = all(counter == counters[0] for counter in counters)
        if not is_equal:
            logger.success(f'any two NIDSs produce inconsistent alerts: {selected_rules}')
            return False

        return True  # If equal, it is a normal behavior, otherwise an alert discrepancy
                        ...
                        while True:
                            ...
                            if line:
                                ...
                                if alert:
                                    ...
                                    logger.info(f'found an alert {alert}')
                                start_time = time.time()
                            else:
