...
from preprocessing.rule_parser.options import Flowbits
...
class RuleSet:
    ...
    def from_files(cls, file_paths: list[str], proto: str = None):
        ...
        for file_path in file_paths:
            logger.info(f"loading rule file: {file_path}")
            ruleset = RuleSet.from_file(file_path)
            logger.info(f"{str(ruleset)}")
        ...
        if proto is not None:
            logger.info(f"grouping rules based on criteria: protocol={proto}")
            rule_pool = rule_pool.group(service=proto)
            logger.debug(f"{str(rule_pool)}")
...
if __name__ == '__main__':
    ...
    print(f"Info: {ruleset}")
    print(f"Flowbits: {ruleset.flowbits}")

    tcp_ruleset = ruleset.group(protocol='TCP', port=None, service=None)
    print(f"TCP Rules Info: {tcp_ruleset}")
    print(f"Flowbits: {tcp_ruleset.flowbits}")
