...
from preprocessing.rule_parser.options import Option, Flow, Content, Isdataat, Pcre, Bufferlen, Dsize, ByteTest
from preprocessing.rule_parser.macro import STICKY_BUFFER
...
class Rule:
    ...
    def from_string(cls, rule_str: str) -> 'Rule':
        ...
        Note: only Traditional Rules are implemented, and three new rule types, i.e., Service Rules,
...
class RuleBody(dict):
    ...
    # TODO: ONLY a few rule options will "move" the detection cursor.
    sig_options = {
        "content": Content,
        "isdataat": Isdataat,
        "pcre": Pcre,
...
if __name__ == "__main__":
    ...
    rule_str = (r'alert tcp $EXTERNAL_NET $HTTP_PORTS -> $HOME_NET any '
                ...
                r'file_data; '
                r'content:"<svg",nocase; content:"<text id",within 400,nocase; content:"getElementByID",within 300,nocase; content:"removeChild",within 100; content:"getCharNumAtPosition",within 200,nocase; '
