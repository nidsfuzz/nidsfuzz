...
from rule_handler import Rule, RuleSet
import logger
from rule_handler.options import Content, Pcre, Isdataat, Option
...
class GenericBufferConstructor(BufferConstructor):
    ...
    def _push_option(self, option: Option, strict: bool = True) -> bool:
        ...
        if isinstance(option, Content):
            ...
            logger.info(f"Not supported option - {type(option)}")
            return True

    def _push_opt_content(self, content: Content, strict: bool = True) -> bool:
        logger.info(f"ContentOption found: {content}.")
        ...
        if content['negated']:
            for character in content.ascii_matches:
                if ord(character) in self._padding_character_set:
                    logger.info(f"\tDelete character [{character.encode('latin-1')}] from padding character set.")
                    self._padding_character_set.remove(ord(character))
                else:
                    logger.info(f"\tCharacter [{character.encode('latin-1')}] not in padding character set.")
            return True
        ...
        if not content["offset"] and not content["depth"] and not content["distance"] and not content["within"]:
            ...
            logger.info(f"\t[{content.ascii_matches.encode('latin-1')}] at [{start_index}] - No modifiers.")
        ...
        logger.info(f"\tCurrent available positions: [{available_positions}]")
        chosen_range = None
        for position in available_positions:
            ...
            if content['offset'] or content['depth']:
                ...
                logger.info(f"\tAbsolute modifiers: [{valid_range_start}] to [{valid_range_end}]")
            ...
            if content['distance'] or content['within']:
                logger.info(f"\tPrevious payload element end index: [{self._previous_payload_element_end_index}]")
                logger.info(f"\tRelative modifiers: [{content['distance']}] to [{content['within']}]")
                ...
                if chosen_range:
                    ...
                    overlap_range_end = valid_range_end if position[1] == -1 else min(valid_range_end, position[1])
                if overlap_range_end != -1 and overlap_range_end - overlap_range_start < len(content.ascii_matches):
                    logger.info(f"\tNot suitable: [{position[0]}] to [{position[1]}].")
                    continue
                chosen_range = (overlap_range_start, overlap_range_end)
        if chosen_range:
            logger.info(f"\t[{content.ascii_matches.encode('latin-1')}] at [{chosen_range[0]}].")
            ...
            logger.info(f"\tAdd signature failed: no suitable position.")
            return False

    def _push_opt_pcre(self, pcre: Pcre, strict: bool = True):
        logger.info(f"PcreOption found: {pcre}.")

        # TODO: check the `negated` modifier more carefully.
        if pcre['negated']:
            logger.info(f"\tSkip signature - Negated.")
            return True

        try:
            data = exrex.getone(pcre['match'])
        except:
            logger.info(f"\tGenerating data from regex failed [{pcre['match']}].")
            return True

        if pcre['R']:
            # If `R` flag exists, put it at last.
            start_index = self._previous_payload_element_end_index
            logger.info(f"\t[{data.encode('latin-1')}] at [{start_index}] - Pcre with `R`.")
            ...
            logger.info(f"\t[{data.encode('latin-1')}] - Pcre without `R`.")
            self.payload_pcre_list.append(data)
            # For better satisfy the rule, also put it at last.
            start_index = self._previous_payload_element_end_index
            logger.info(f"\t[{data.encode('latin-1')}] at [{start_index}] - Pcre without `R`.")
            self._set_previous_payload_element_end_index(self._previous_payload_element_end_index, data)
            self.payload_element_list.append(self.PayloadElement(start_index, data))
        return True

    def _push_opt_isdataat(self, isdata: Isdataat, strict: bool = True):
        logger.info(f"IsdataatOption found: {isdata}.")
        if isdata['negated']:
            ...
            if self._payload_length_min is None or self._payload_length_min <= payload_length_max:
                ...
                logger.info(f"\tAdd signature succeed: payload_length_max to [{payload_length_max}].")
                return True
            else:
                logger.info(
            ...
            if self._payload_length_min is not None and payload_length_min <= self._payload_length_min:
                logger.info(f"\tAdd signature succeed: payload_length_min [{payload_length_min}] ignored.")
                return True
                ...
                logger.info(f"\tAdd signature succeed: payload_length_min to [{payload_length_min}].")
                return True
            else:
                logger.info(
    ...
    def get_payload(self, from_pcre=False, force_pcre=False) -> str:
        ...
        if self._payload_length_min and self._payload_length_max:
            if len(payload) < self._payload_length_min:
                logger.info(f"Padding payload. [{len(payload)}] to [{self._payload_length_min}]")
                ...
                logger.info(f"Payload too long. [{len(payload)}] to [{self._payload_length_max}]")
...
class RuleCombineStrategy(MutateStrategy):
...
def test_sig_combine(rule_file_path: str, protocol: str):
...
if __name__ == "__main__":
    rule_file = os.path.join(
        ...
        "rules",
        "snort3-browser-firefox.rules.one"
    )
    test_sig_combine(rule_file, "HTTP")
